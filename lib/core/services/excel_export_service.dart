...
class ExcelExportService {
  static Future<String> exportTimeEntries({
    ...
    for (var entry in entries) {
      var date = DateTime(entry.startTime.year, entry.startTime.month, entry.startTime.day);
    ...
    for (var date in sortedDates) {
      ...
      reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row))
          .value = TextCellValue('${date.day}/${date.month}/${date.year}');
      ...
      for (var entry in dayEntries) {
        ...
        var duration = entry.endTime?.difference(entry.startTime) ?? Duration.zero;
        ...
        // Project
        reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: row))
            .value = TextCellValue(project?.name ?? 'Unknown Project');
        ...
        final fullTaskName = entry.description.trim().isNotEmpty 
            ...
            : taskName;
        reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: row))
            .value = TextCellValue(fullTaskName);
            
        // Start Time
        reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: row))
            .value = TextCellValue(TimeFormatter.formatTime(entry.startTime));
            
        // End Time
        reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: row))
            .value = TextCellValue(entry.endTime != null ? TimeFormatter.formatTime(entry.endTime!) : 'In Progress');
            
        // Duration
        reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: row))
            .value = TextCellValue(TimeFormatter.formatDuration(duration));
            
        // Duration in hours (for calculations)
        reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row))
            .value = DoubleCellValue(duration.inMinutes / 60.0);
            
        // Status
        reportSheet.cell(CellIndex.indexByColumnRow(columnIndex: 7, rowIndex: row))
            .value = TextCellValue(entry.isCompleted ? 'Completed' : 'In Progress');
  ...
  static void _createSummarySheet(Sheet sheet, List<TimeEntry> entries, Map<String, Project> projects) {
    // Set up headers
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 1)).value = TextCellValue('Summary Report');
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 3)).value = TextCellValue('Project');
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: 3)).value = TextCellValue('Total Tasks');
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: 3)).value = TextCellValue('Total Hours');
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: 3)).value = TextCellValue('Avg Hours/Task');
    ...
    for (var stats in projectStats.values) {
      ...
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: row))
          .value = IntCellValue(taskCount);
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: row))
          .value = DoubleCellValue(double.parse(hours.toStringAsFixed(2)));
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: row))
          .value = DoubleCellValue(double.parse(avgHours.toStringAsFixed(2)));
    ...
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: row))
        .value = IntCellValue(totalTasks);
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: row))
        .value = DoubleCellValue(double.parse(totalHours.toStringAsFixed(2)));
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: row))
        .value = DoubleCellValue(totalTasks > 0 ? double.parse((totalHours / totalTasks).toStringAsFixed(2)) : 0.0);
  }
  ...
  static String _generateFileName(DateTimeRange? dateRange) {
    ...
    var dateStr = '${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')}';
    
    if (dateRange != null) {
      var startStr = '${dateRange.start.year}-${dateRange.start.month.toString().padLeft(2, '0')}-${dateRange.start.day.toString().padLeft(2, '0')}';
      var endStr = '${dateRange.end.year}-${dateRange.end.month.toString().padLeft(2, '0')}-${dateRange.end.day.toString().padLeft(2, '0')}';
  ...
  static Future<String> _saveExcelFileWithFileSaver(Excel excel, String fileName) async {
