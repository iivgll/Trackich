...
Future<Map<String, List<TaskGroupWithProject>>> filteredTimeEntries(
  ...
  for (final group in filteredGroups) {
    try {
      final project = activeProjects.firstWhere((p) => p.id == group.projectId);
      ...
      // Skip groups with archived or deleted projects
    }
  }
                    ...
                    children: [
                      ...
                      if (taskGroupWithProject.taskGroup.sessionCount > 1) ...[
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: AppTheme.focusPurple.withValues(alpha: 0.1),
...
class _TaskGroupDetailsDialog extends ConsumerWidget {
  ...
  Future<void> _continueTask(
    ...
    if (!taskGroupWithProject.project.isActive) {
      // Show error message for archived project
      ScaffoldMessenger.of(ref.context).showSnackBar(
        SnackBar(
          content: Text(
            'Cannot continue task: Project "${taskGroupWithProject.project.name}" is archived',
          ),
    ...
    final errorColor = Theme.of(ref.context).colorScheme.error;
    ...
    try {
      ...
      scaffoldMessenger.showSnackBar(
        SnackBar(
          content: Text('Error continuing task: $e'),
